...
deps:
	...
	#cd /tmp/govendor/src/github.com/bsm/
	#rm -rf sarama-cluster
	#git clone  https://github.com/bsm/sarama-cluster.git
	mkdir -p /tmp/govendor/bin
	mkdir -p /go/src/gitlab.wallstcn.com/operation/
	cp -R "/builds/operation/$(SERVICE_NAME)" "/go/src/gitlab.wallstcn.com/operation/$(SERVICE_NAME)/"
test:
	go test
